---
title: "Exploratory Data Analysis from R4DS"
...
[Exploratory Data Analysis](https://r4ds.had.co.nz/exploratory-data-analysis.html) chapter from R for Data Science book.
...
```{r cv2, fig.width = "50%", fig.width = 4, exercise = TRUE}
ggplot(diamonds) + 
  geom_bar(mapping = aes(x = cut))
```
...
```{r cv4, fig.height = 3, exercise = TRUE}
...
```{r cv6, fig.height = 3, exercise = TRUE}
ggplot(data = mpg) +
...
```{r tcv6, fig.asp = 1, out.width = "50%", fig.align = "default", message = FALSE, exercise = TRUE}
ggplot(data = smaller) +
...
```{r pm4, eval = FALSE, exercise = TRUE}
ggplot(data = faithful, mapping = aes(x = eruptions)) + 
...
```{r pm5, eval = FALSE, exercise = TRUE}
...
```{r pm6, eval = FALSE, exercise = TRUE}
diamonds %>% 
  count(cut, clarity) %>% 
